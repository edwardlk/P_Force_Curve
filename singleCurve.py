...
def multiLinReg(x_data,y_data):
	...
	returns contact slope, contact intercept, baseline slope, baseline intercept"""
	# print "Running MLR"
	sepX = 1
	deltaX = 0.0
	while deltaX < 50:
	...
	for x in range(1999):
			s1, i1, r1, p1, se1 = stats.linregress(x_data[:int(init_points[x,0])], y_data[:int(init_points[x,0])])
			s2, i2, r2, p2, se2 = stats.linregress(x_data[int(init_points[x,0]):], y_data[int(init_points[x,0]):])
	...
	s1, i1, r1, p1, se1 = stats.linregress(x_data[:int(init_points[maxPos,0] - sepX/5)],
										   y_data[:int(init_points[maxPos,0] - sepX/5)])
	s2, i2, r2, p2, se2 = stats.linregress(x_data[int(init_points[maxPos,0] + sepX):],
...
baselineS, baselineI, baselineR, baselineP, baselineE = stats.linregress(
        retractZ[int(Rbounds[1,0]):int(Rbounds[0,0])], retractD[int(Rbounds[1,0]):int(Rbounds[0,0])])
contactS, contactI, contactR, contactP, contactE = stats.linregress(
        retractZ[int(Rbounds[3,0]):int(Rbounds[2,0])], retractD[int(Rbounds[3,0]):int(Rbounds[2,0])])
